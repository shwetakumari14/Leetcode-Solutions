...
app.get('/events_details/:id', async (req, res) =>{
  ...
  request(endpoint, function (error, response, body) {
    if (!error && response.statusCode == 200) {
      ...
      if (apiResponse.priceRange != null){
        if (apiResponse.priceRange.length > 0){
          if(apiResponse.priceRange[0].min != null && apiResponse.priceRange[0].max != null){
            priceRange = str(int(apiResponse.priceRange[0].min)) + " - " + str(int(apiResponse.priceRange[0].max)) + "USD"
          }
        }
      }
